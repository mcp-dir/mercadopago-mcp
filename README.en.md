...
### Mercado Pago in Claude, ChatGPT and AI agents via Open Finance

Connect your **Mercado Pago** account to Claude, ChatGPT and AI agents via Brazil's Open Finance. Ask about Mercado Pago balances, statements, credit-card bills and investments in natural language. Regulated by the Central Bank, read-only.

- 🏦 **Mercado Pago via Brazil's Open Finance**, regulated by the Central Bank
...
[➕ Open in Claude and connect](https://claude.ai/customize/connectors?modal=add-custom-connector&mcpName=Mercado%20Pago%20MCP&mcpServerUrl=https%3A%2F%2Fapi.mcp.ai%2Fp_mercadopago)

Manual: [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Add custom connector** → name `Mercado Pago MCP`, URL `https://api.mcp.ai/p_mercadopago`.

### Cursor

[➕ Install in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=mercadopago&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9tZXJjYWRvcGFnbyJ9)

### VS Code (Copilot Chat)

[➕ Install in VS Code](vscode:mcp/install?name=mercadopago&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_mercadopago%22%7D)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_mercadopago` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
