...
### Mercado Pago no Claude, ChatGPT e agentes de IA via Open Finance
...
- 🏦 **Mercado Pago via Open Finance Brasil**, regulado pelo Banco Central
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Adicionar conector personalizado** → cole **Nome** `Mercado Pago MCP` e **URL** `https://api.mcp.ai/p_mercadopago`.

### Cursor

[➕ Instalar Mercado Pago MCP no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=mercadopago&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9tZXJjYWRvcGFnbyJ9)

### VS Code (Copilot Chat)

[➕ Instalar Mercado Pago MCP no VS Code](vscode:mcp/install?name=mercadopago&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_mercadopago%22%7D)
...
```
Quanto gastei no cartão do Mercado Pago esse mês?
Resuma minha fatura do Mercado Pago em aberto
Qual meu saldo na conta do Mercado Pago?
Como está minha carteira de investimentos no Mercado Pago?
```
...
**Como funciona?**
Via Open Finance Brasil (regulado pelo Banco Central). Você conecta sua conta Mercado Pago com consentimento explícito; a IA lê saldos, extratos, faturas e investimentos. Somente leitura.

**A IA pode movimentar dinheiro?**
Não. 100% leitura — Open Finance não permite movimentação por essa via.

**Só funciona com o Mercado Pago?**
Este MCP é focado no Mercado Pago. Para conectar vários bancos de uma vez, use o Banco MCP (api.mcp.ai/banco).
...
- 📧 [mercadopago@mcp.ai](mailto:mercadopago@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/mercadopago-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_mercadopago` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
